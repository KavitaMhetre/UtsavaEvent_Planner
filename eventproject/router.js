...
router.get('/get-events', signupValidation, (req, res, next) => {
    ...
    db.query('SELECT * FROM events as e left join users_vendor as u on e.vid = u.id where uid=?', decoded.id, function (error, results, fields) {
...
router.get('/vendor-all-events', signupValidation, (req, res, next) => {
    ...
    // const theToken = req.headers.authorization.split(' ')[1];
    // const decoded = jwt.verify(theToken, 'the-super-strong-secrect');
    db.query(`SELECT *, e.id as eid FROM events as e join users as u on e.uid = u.id where vid=''`, function (error, results, fields) {
        if (error) throw error;
        return res.send({ error: false, data: results, message: 'Fetch Successfully.' });
    });
});
